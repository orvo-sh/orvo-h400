...
RETURNING
    id, token, user_id, active_organization_id, ip_address, user_agent, expires_at, created_at, updated_at
`
...
const getSessionWithUserAndOrganization = `-- name: GetSessionWithUserAndOrganization :one
SELECT
    s.id as session_id,
    s.token,
    s.user_id,
...
type GetSessionWithUserAndOrganizationRow struct {
	SessionID            string             `json:"session_id"`
	Token                string             `json:"token"`
	UserID               string             `json:"user_id"`
	ActiveOrganizationID pgtype.Text        `json:"active_organization_id"`
	...
	OrgUpdatedAt         pgtype.Timestamptz `json:"org_updated_at"`
	MemberRole           pgtype.Text        `json:"member_role"`
}

func (q *Queries) GetSessionWithUserAndOrganization(ctx context.Context, token string) (GetSessionWithUserAndOrganizationRow, error) {
	row := q.db.QueryRow(ctx, getSessionWithUserAndOrganization, token)
	var i GetSessionWithUserAndOrganizationRow
	err := row.Scan(
		&i.SessionID,
		&i.Token,
		&i.UserID,
