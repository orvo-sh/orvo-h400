...
func (m *SandboxManager) processJob(ctx context.Context, jobID string) error {
	...
	for _, command := range jobCtx.Commands {
		...
		result, runErr := m.execWithHeartbeat(execCtx, session, jobID, "cd repo && "+command.Command, fmt.Sprintf("command #%d", command.Ordinal), nil, m.config.CommandTimeout)
		if runErr != nil {
	...
	prBody := fmt.Sprintf("Automated changes generated by Orvo sandbox job `%s`.", jobID)
