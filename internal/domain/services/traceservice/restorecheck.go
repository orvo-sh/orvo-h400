...
func (s *service) checkRestoreRequired(ctx context.Context, input QueryTracesInput) apperr.Error {
	...
	covered, err := s.fetchCoveredDays(ctx, input.OrganizationID, "traces", olderDays)
	if err != nil {
	...
	restorable, err := s.fetchArchiveDays(ctx, input.OrganizationID, "traces", missing)
	if err != nil {
		return errs.ErrInternal
	}
	if len(restorable) == 0 {
	...
	jobID, jobState, err := s.fetchPendingRestoreJob(ctx, input.OrganizationID, "traces", startDay, endDay)
	if err != nil {
		return errs.ErrInternal
	}
...
func dayInClause(days []time.Time, args *[]any, startIdx int) string {
	...
	return stringsJoin(placeholders)
}
...
func stringsJoin(items []string) string {
	...
	return fmt.Sprintf("%s", joinWithComma(items))
}
